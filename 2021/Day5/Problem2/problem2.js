    ...
    reader.onload= async function() {
        ...
        for (let coord of coords) {
            if (coord[0][1] === coord[1][1]) {
                ...
                const xDir = coord[0][0] < coord[1][0] ? 1 : -1;
                const yDir = coord[0][1] < coord[1][1] ? 1 : -1;

                let y = coord[0][1];
                for (let x = coord[0][0]; x !== coord[1][0]; x += xDir) {
                    grid[x][y]++;
                    if (grid[x][y] === 2) {
                        answer++;
                    }
                ...
                grid[coord[1][0]][coord[1][1]]++;
                if (grid[coord[1][0]][coord[1][1]] === 2) {
                    answer++;
                }
            }
        }
