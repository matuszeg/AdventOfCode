...
document.getElementById('inputfile').addEventListener('change', function () {
    ...
    reader.onload= async function() {
        ...
        let numSteps = 0;
        let numStepFlashes = 0;
        const numTargetFlashes = inputs.length * inputs[0].length;
        while (numStepFlashes !== numTargetFlashes && numSteps < 10000) {
            document.getElementById("currentStep").innerText = (numSteps+1) + "";
            numStepFlashes = await Step(inputs);
            await Render(inputs);
            numSteps++;
        }

        //The answer is just numSteps not numSteps+1 because it auto increments at the end of the loop
        const answer = numSteps;
